...
## 3. 第二个方案
为了有序管理，“10010店铺”改进了一下，把相同加热时间的放在一排，这样一旦加热完成，只会有一排炉子需要处理。这样机器人就轻松多了。
...
* 机器人手里拿着一张“登记表”，上面记录：
  * 2020年3月1日10点15分6秒：第一排
  * 2020年3月1日10点8分0秒：第二排
  * ...
  * 2020年3月1日10点9分12秒：第N排
* 加热时间一到，机器人听到一声叮咚响声：
  * 机器人查看当前时间为2020年3月1日10点8分0秒
  * 机器人查看自己手里的“登记表”，查到是第二排
* 机器人走到第二排，把这一排所有的炉子中的食物取出，交给客户
...
* 每创建一个timer对象，nodejs会先计算出该timer对象的到期时间（绝对时间）。
* 然后根据到期时间（绝对时间），创建一个链表list（TimersList实例），并把该timer对象放到此链表中。
* 给新创建的链表分配一个标识。其实就是往一个公用对象timerListMap中，设置一个key:value，key是到期时间（绝对时间），value是链表本身

这样，后续再创建新的timer对象，nodejs还是先计算出到期时间（绝对时间），然后去timerListMap查找有没有对应到期时间的链表。如果有，则直接插入到对应的链表中。如果没有，则重新创建链表，并塞到timerListMap中。
