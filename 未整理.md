  ...
  * 伪终端
    ...
    * unix98 伪终端 api
      ...
      * 其实还有两个api, 这两个api，其实都是对伪终端从设备 进行操作的。但是从api 上看不出来，很容易引起歧义
        * int grantpt(int mfd): 用来修改 【mfd代表的伪终端对中的从设备】 的属主和权限
...
* strace
  * strance command arg... 允许我们跟踪程序执行的系统调用
  * 默认情况下，strace 会将输出写入到 stderr 中，但我们可以通过-o filename 的选项来修改这个行为。
  * ![strace](./img/strace.png)
  * 如果想要过滤，可以使用 以下两种形式进行过滤
    * 通过 grep 过滤：strance date 2>&1 | grep open
    * 通过 -e 指定过滤哪些系统调用 strance -e trace=open,close date
* 解析命令行参数
  * 一个典型的命令行如下：command [options] args
    * options: 选项
      * 随着后来的发展，选项也需要动态化和扩展，因此选项也有自己的参数
      * 选项是用 - 来标识
    * args: 参数
      * 其实这个是真正的参数。但是因为opt 也支持了参数，所以这里的参数倒是被改了称谓，叫非选项参数
      * 有点鸠占鹊巢的意思。
    * 样例：
      ...
      * grep -lifpatterns *.c
        * 为什么这种形式的也能识别呢？
        * 大多数都是基于规范，通过 getopt 的第三个参数来指定选项的名称
        * 基本上都是 - 加一个单字符来标识 选项。
    * int getopt(int argc, char *const argv[], const char *optstring)
      * argc 和 argv 是main 函数传递进来的。
      * 参数 optstring 指定了函数 getopt()应该寻找的命令行选项集合
        * 应用程序就是通过 这个 optstring 来表明 命令支持哪些选项的
          * 是不是有点感觉反过来了，可能会以为 用户手动输入是第一步，执行getopt 是被动接受。
          * 现在的一个事实确实，通过getopt 的第三个参数来规定 用户可以输入哪些
          * 如果用户输入的不遵守 getopt 的第三个参数，那么该函数将会报错（返回一个 ？）
      * 通过循环调用该函数，来依次解析。
        * 返回值是选项，注意这里是返回选项的int 形式，程序可以使用printable(int) 来转为字符串。
        * 因为每次传入的参数都一样，那么怎么区分第几次调用，以及获取到调用结果呢？
        * 答案是有两个全局变量：
          * optind: 表示下一个要解析的argv index（会自动跳过 选项参数）
            * 样例：
            * command1 -l larg arg1
            * 调用 index = getopt(argc, argv, "l:"), 此时 index 将是3，自动跳过 -l 的参数 larg
            * 所以 argv[index] === argv[3] === 'arg1'
            * 号外：
              * argv[1][0] === "l"
              * argv[1][1] === "larg"
          * optarg: 如果当前选项带有参数，那么此次调用getopt，全局变量 optarg 将会指向该选项的参数
        * 如果getopt 处理完了第三个参数optstring 指定的选项，再次调用，将会返回 -1
          * 此时optind 因为是指向下一个 下表，因此 此时 argv[optind] 将会返回 第一个非选项参数
          * 返回 -1 的情形：
            * 已经到达argv 列表的结尾
            * 下一个要处理的，不是以 - 开头的。
            * 下一个要处理的，是以 -- 形式的值
  * 特定于 GNU 的行为
    * glibc 版的 getopt()实现还有一个非标准的功能：允许选项和非选项混在一起
    * ls -l file === ls file -l
      * 底层实现时：getopt 先对 ls 后面的值（opt + args）进行重排，opt 放到前面
* 内核配置
  * linux 内核的很多特性是通过组件组装得来的（除了一些非常核心的默认且必须，其他的可以通过配置来确定是否编译进来）  
  * 怎么指定呢？就是通过一个 根目录下的 ./config 文件
  * 如何产生 ./config 文件
    * 最笨的方式：手工编写
    * 通过在根目录下运行 make menuconfig： 提供易用性不太好的配置菜单
    * 通过在根目录下运行 make xconfig: 提供易用性较好的图形配置菜单
  * ./config 在哪个阶段发生作用
    * 在编译可用的linux 内核二进制文件时，在编译阶段会读取这个文件
    * 构建完成后，会往 /proc/config.gz 写一份真正编译生效的文件
      * 这个文件内容，基本和 ./config 内容一样
      * 因此，用户可以直接通过 查看 /proc/config.gz 来获取当前可运行的linux 内核有哪些组件
      * 可以通过zcat 来查看这个文件
  * ./config 文件内容是啥？
    * 每一行代表一个功能组件，格式为 CONFIG_NAME=value
  * 构建
# gc
...
# cpu
  ...
  * 协程
    ...
    * 本质就是通过 各个寄存器的备份和恢复，来完成上下文的切换
      ...
      * 参见glibc 的几个函数。
        * getcontext(&context): 保存当前各个寄存器的值到参数 context 中
        * setcontext(&context): 把 context 中的对应值，覆盖当前各个寄存器的值
          * restore the user context by &context
          * a succ call never return, but the program resume at the point specified by the &context
            * 意思就是调用 setcontext 后，由于对应的寄存器的值进行了调整，所以接下来要执行的指令就有 &context 指定的值来指定
            * 如果 &context 是通过 getcontext 获取的，且没有任何改变，那么调用setcontext后，程序就相当于 getcontext 刚刚运行完，即此时要运行getcontext 的下一个指令
            * 如果 手工调整了 &context，比如指定了新的栈大小，并设置了fun (makecontext)，那么调用完setcontext 后，将会运行makecontext 中制定的fun
              * 当这个fun 结束时，又分两种情况：
                * 如果这个 &context.uc_link 不为空，那么程序将指向 &context.uc_link 指向的新context
                * 如果这个 &context.uc_link 为空，那么程序将指向 另外一个context, 也就是 main context，意味着程序将 从main 函数最开始setcontext或者swapcontext时的地方（此时也是main context 保持的地方），接着执行，然后exit
          * 可以说，getcontext 是 setcontext 相对操作 
          * 二者结合起来，便可以实现 goto 类似的能力
        * https://blog.csdn.net/mrpre/article/details/78699865
        * https://www.ibm.com/docs/en/zos/2.2.0?topic=functions-setcontext-restore-user-context
    * 调度策略：
